<template>
  <card :title="$t('project_info_update')">
    <form @submit.prevent="update" @keydown="send.onKeydown($event)">
      <alert-success :form="send" :message="$t('project_info_update_success')" />
...
<script>
  ...
  export default {
    ...
    created () {
      this.big_project_info_list()
      this.project_info_list()
    },

    methods: {
      async big_project_info_list () {
        var data = await this.form.get('/api/big_project_info/list')
        this.big_id_option = data.data.data.list
      },
      async project_info_list (id) {
        this.data.id = this.$route.params.id;
        var data = await this.data.post('/api/project_info/list')
      ...
      async update () {
        ...
        const { data } = await this.send.post('/api/project_info/update')
